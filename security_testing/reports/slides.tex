...
\begin{column}{0.48\textwidth}
  ...
  \begin{itemize}\small
    ...
    \item Boolean connector (40)
    \item Bitwise operator (40)
...
\begin{frame}{Enhanced Coverage with LCOV Artifacts}
\centering
...
\begin{itemize}
  \item Mutation adequacy (69.67\%) --- primary area for improvement
  \item All other dimensions achieved \textbf{80\%+ pass rates}
  \item Core parsing and protocol handling is \textbf{robust}
\end{itemize}
\end{frame}

% ─────────────────────── SLIDE 19 : CROSS-PLATFORM ───────────────────────
\begin{frame}{Cross-Platform Portability}
\begin{block}{Dual-platform execution}
\textbf{8 of 9} test suites executed successfully on both Windows and Ubuntu.
\end{block}
\vspace{0.3cm}
\begin{itemize}
  \item \textbf{Sole exception:} Fuzzing blocked on Windows (MSVC/libFuzzer issue)
  \item Performance harness used custom in-memory loopback for fair comparison
  \item Results underscore importance of multi-toolchain testing
\end{itemize}
\end{frame}
...
\begin{frame}[plain]
  ...
  \vspace{0.5cm}
  {\Large\color{accentlime} Questions?\par}
  \vspace{1.5cm}
